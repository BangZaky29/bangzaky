  ...
  return (
    <div className="min-h-screen bg-bg-main text-text-primary font-sans selection:bg-accent-primary selection:text-bg-main flex flex-col relative overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-50 px-4 md:px-8 py-6">
        <div className="max-w-7xl mx-auto bg-bg-panel/90 backdrop-blur-xl rounded-2xl border border-border-soft shadow-lg px-6 py-4 flex items-center justify-between transition-all duration-300">
          <NavLink to="/" className="flex items-center gap-3 group">
            <div className="w-10 h-10 bg-bg-elevated rounded-xl flex items-center justify-center border border-border-strong shadow-2-5d group-hover:shadow-glow transition-all duration-300">
              <Terminal className="text-accent-primary w-5 h-5" />
            </div>
            <span className="font-bold text-xl tracking-tight text-text-primary">Bang<span className="text-accent-primary">Zaky</span></span>
          </NavLink>

          {/* Desktop Nav */}
          <div className="hidden md:flex items-center gap-8">
            <div className="flex items-center gap-1 bg-bg-main/50 p-1.5 rounded-xl border border-border-soft/50 shadow-inner">
              {NAV_ITEMS.map((item) => (
                <NavLink 
                  key={item.path} 
                  to={item.path}
                  className={({ isActive }) => 
                    `px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${
                      isActive 
                        ? 'bg-bg-elevated text-accent-primary shadow-sm border border-border-soft' 
            ...
            <div className="h-6 w-px bg-border-soft/50" />
             ...
             <NavLink 
               to="/dashboard" 
               className={({ isActive }) => `relative group p-2 rounded-lg transition-all ${isActive ? 'bg-bg-elevated text-accent-primary shadow-sm' : 'text-text-muted hover:text-text-primary'}`}
             >
          ...
          {/* Mobile Menu Toggle */}
          <button 
            className="md:hidden p-2 text-text-muted hover:text-accent-primary hover:bg-bg-elevated rounded-lg transition-colors"
      ...
      {/* Content */}
      <main className="flex-grow px-4 md:px-8 py-8 md:py-12 relative z-10">
      ...
      {/* Footer */}
      <footer className="border-t border-border-soft bg-bg-surface mt-auto relative z-10">
